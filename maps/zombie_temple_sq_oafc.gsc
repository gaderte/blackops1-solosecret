...
oafc_trigger_thread_solo(tiles, set)
{
	self endon("death");
	level endon("reset_tiles");
						...
						if(set == 1 || set == 2)
						{
							if(IsDefined(level._picked_tile1) && IsDefined(level._picked_tile2))
							{
								if(level._picked_tile1.tile == level._picked_tile2.tile)
								{
									...
									// Si on a match une paire mais que c'est pas fini
									...
									// Si tout est terminé => On a match les 12 paires
									if(level._num_matched_tiles == level._num_tiles_to_match)
									{
										struct = getstruct( "sq_location_oafc", "targetname" );
										if( isdefined( struct ) )
...
oafc_trigger_thread_mp(tiles, set)
{
	self endon("death");
	level endon("reset_tiles");
...
reset_tiles()
{
	...
	if(get_players().size == 1)
	{
		level._oafc_trigger1 thread oafc_trigger_thread_solo(tileset1,1);
		level._oafc_trigger2 thread oafc_trigger_thread_solo(tileset2,2);
	}
	else {
		level._oafc_trigger1 thread oafc_trigger_thread_mp(tileset1,1);
		level._oafc_trigger2 thread oafc_trigger_thread_mp(tileset2,2);
	}
